...
public interface RoleClient {
	...
	ApiResponse<Page<RoleDto>> getRoles(@PageableDefault(page = 1, size = 10) Pageable pageable);

	@GetMapping("/back/roles/{roleId}")
	ApiResponse<RoleDto> getRole(@PathVariable Long roleId);

	@PostMapping("/back/roles")
	ApiResponse<Void> createRole(@RequestBody RoleDto request);

	@PutMapping("/back/roles/{roleId}")
	ApiResponse<Void> updateRole(@PathVariable Long roleId, @RequestBody RoleDto request);
