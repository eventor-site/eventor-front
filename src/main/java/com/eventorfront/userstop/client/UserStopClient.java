...
public interface UserStopClient {
	...
	ApiResponse<Page<GetUserStopResponse>> getUserStops(@PageableDefault(page = 1, size = 10) Pageable pageable);

	@GetMapping("/back/userStops/users")
	ApiResponse<List<GetUserStopByUserIdResponse>> getUserStopsByUserId(@RequestParam Long userId);

	@GetMapping("/back/userStops/{userStopId}")
	ApiResponse<UserStopDto> getUserStop(@PathVariable Long userStopId);

	@PostMapping("/back/userStops")
	ApiResponse<Void> createUserStop(@RequestBody UserStopDto request);

	@DeleteMapping("/back/userStops/{userStopId}")
	ApiResponse<Void> deleteUserStop(@PathVariable Long userStopId);
}
