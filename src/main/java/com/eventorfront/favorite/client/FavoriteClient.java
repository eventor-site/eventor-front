...
public interface FavoriteClient {
	...
	ApiResponse<Page<GetFavoriteResponse>> getFavoritesByUserId(
		@PageableDefault(page = 1, size = 10) Pageable pageable);

	@PostMapping("/back/post/{postId}/favorites")
	ApiResponse<Void> createOrDeleteFavorite(@PathVariable Long postId);

	@DeleteMapping("/back/{favoriteId}")
	ApiResponse<Void> deleteFavorite(@PathVariable Long favoriteId);
}
